...
RUN groupadd -r -g $GROUP_ID drobo && \
    useradd -r -u $USER_ID -g drobo -G sudo drobo && \
    echo drobo:drobo | chpasswd

RUN wget -O /tmp/SDK-2.1.zip ftp://updates.drobo.com/droboapps/development/SDK-2.1.zip && \
...
RUN wget -O /tmp/arm-drobo_x86_64-linux-gnueabi.tar.xz https://github.com/drobo/cross-compiler/releases/download/v${XTOOL_VERSION}/arm-drobo_x86_64-linux-gnueabi.tar.xz && \
...
RUN wget -O /tmp/xpython2.tgz https://github.com/droboports/python2/releases/download/v${PYTHON_VERSION}/xpython2.tgz && \
...
RUN wget -O /tmp/xgolang.tgz https://github.com/droboports/golang/releases/download/v${GOLANG_VERSION}/xgolang.tgz && \
    mkdir -p /home/drobo/xtools/golang/5n && \
    tar -zxf /tmp/xgolang.tgz -C /home/drobo/xtools/golang/5n && \
    rm -f /tmp/xgolang.tgz && \
    chown -R drobo:drobo /home/drobo

RUN mkdir -p   /mnt/DroboFS/Shares/DroboApps /mnt/DroboFS/System /dist && \
